...
""",
    "Countries": """
afghanistan albania algeria andorra angola argentina armenia australia austria azerbaijan bahamas bahrain bangladesh barbados belarus belgium belize benin bhutan bolivia botswana brazil brunei bulgaria burundi cambodia cameroon canada chad chile china colombia comoros congo croatia cuba cyprus czechia denmark djibouti dominica ecuador egypt elsalvador eritrea estonia ethiopia fiji finland france gabon gambia georgia germany ghana greece grenada guatemala guinea guyana haiti honduras hungary iceland india indonesia iran iraq ireland israel italy jamaica japan jordan kazakhstan kenya kuwait kyrgyzstan laos latvia lebanon lesotho liberia libya liechtenstein lithuania luxembourg madagascar malawi malaysia maldives mali malta mauritania mauritius mexico micronesia moldova monaco mongolia montenegro morocco mozambique myanmar namibia nepal netherlands newzealand nicaragua niger nigeria norway oman pakistan palau panama papuanewguinea paraguay peru philippines poland portugal qatar romania russia rwanda samoa sanmarino senegal serbia seychelles sierraleone singapore slovakia slovenia somalia spain srilanka sudan suriname sweden switzerland syria taiwan tajikistan tanzania thailand togo tonga trinidad tunisia turkey turkmenistan tuvalu uganda ukraine unitedarabemirates unitedkingdom unitedstates uruguay uzbekistan venezuela vietnam yemen zambia zimbabwe
""",
    "Nationalities": """
afghan albanian algerian andorran angolan argentinian armenian australian austrian azerbaijani bahamian bahraini bangladeshi barbadian belarusian belgian belizean beninese bhutanese bolivian botswanan brazilian bruneian bulgarian burundian cambodian cameroonian canadian chadian chilean chinese colombian comorian congolese croatian cuban cypriot czech danish djiboutian dominican ecuadorian egyptian eritrean estonian ethiopian fijian finnish french gabonese gambian georgian german ghanaian greek grenadian guatemalan guinean guyanese haitian honduran hungarian icelandic indian indonesian iranian iraqi irish israeli italian jamaican japanese jordanian kazakh kenyan kuwaiti kyrgyz laotian latvian lebanese basotho liberian libyan liechtensteiner lithuanian luxembourgish malagasy malawian malaysian maldivian malian maltese mauritanian mauritian mexican micronesian moldovan monacan mongolian montenegrin moroccan mozambican myanmar namibian nepali dutch newzealander nicaraguan nigerien nigerian norwegian omani pakistani palauan panamanian papuan paraguayan peruvian philippine polish portuguese qatari romanian russian rwandan samoan senegalese serbian seychellois sierraleonean singaporean slovak slovenian somali spanish srilankan sudanese surinamese swedish swiss syrian taiwanese tajik tanzanian thai togolese tongan trinidadian tunisian turkish turkmen tuvaluan ugandan ukrainian emirati british american uruguayan uzbek venezuelan vietnamese yemeni zambian zimbabwean
""",
}
...
SLANG_WORDS = {
    ...
    "idiot", "stupid", "crazy", "insane", "bastard",
...
def official_category(english: str) -> str:
    key = english.lower()
    if key in SLANG_WORDS: return "Slang and Swear Words"
    if key in COLOR_WORDS: return "Colors"
    if key in BODY_WORDS: return "Body and Health"
    if key in ACTION_WORDS: return "Common Verbs"
    if key in HOME_WORDS: return "Home and Daily Life"
    if key in {"my", "your", "his", "her", "its", "our", "their", "mine", "yours", "ours", "theirs"}: return "Possessives"
    if key in {"to be"}: return "Grammar Aliases"
    
    words = key.split()
    
    # Thematic categories
    if any(w in words for w in ["dog", "cat", "bird", "fish", "animal", "bear", "horse", "cow", "pig", "sheep", "chicken", "lion", "tiger", "snake", "bug", "insect", "spider", "mouse", "rat", "wolf"]):
        return "Animals"
    if any(w in words for w in ["food", "water", "drink", "eat", "bread", "meat", "fruit", "apple", "banana", "vegetable", "milk", "cheese", "rice", "soup", "cake", "sugar", "salt", "egg", "fish", "tea", "coffee"]):
        return "Food and Drink"
    if any(w in words for w in ["tree", "flower", "grass", "leaf", "plant", "sun", "moon", "star", "sky", "cloud", "rain", "snow", "wind", "storm", "fire", "earth", "dirt", "rock", "stone", "river", "lake", "ocean", "sea", "mountain", "hill", "forest"]):
        return "Nature and Weather"
    if any(w in words for w in ["car", "bus", "train", "plane", "boat", "ship", "road", "street", "bridge", "station", "airport", "drive", "ride", "fly", "travel"]):
        return "Transportation"
    if any(w in words for w in ["computer", "phone", "screen", "machine", "engine", "internet", "website", "software", "hardware", "code", "data", "file", "network"]):
        return "Technology"
    if any(w in words for w in ["person", "people", "man", "woman", "child", "boy", "girl", "baby", "mother", "father", "parent", "brother", "sister", "family", "friend", "king", "queen", "president", "doctor", "teacher", "student", "police", "worker"]):
        return "People and Roles"
    if any(w in words for w in ["time", "day", "night", "morning", "evening", "week", "month", "year", "today", "tomorrow", "yesterday", "hour", "minute", "second", "clock", "calendar", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]):
        return "Time and Calendar"
    
    # Grammatical categories
    if key.startswith("to "): return "Verbs"
    if " " in key: return "Phrases and Expressions"
    if key.endswith("ly"): return "Adverbs"
    if key.endswith("ing"): return "Gerunds (Actions)"
    if key.endswith("ed"): return "Past Tense and States"
    if any(key.endswith(s) for s in ["tion", "sion", "ment", "ness", "ity", "ance", "ence", "ism"]): return "Abstract Nouns"
    if any(key.endswith(s) for s in ["able", "ible", "ous", "ive", "ful", "less", "ic", "al"]): return "Adjectives"
    if any(key.endswith(s) for s in ["er", "or", "ist", "ian"]): return "People and Roles"
    
    # Alphabetical fallback
    first = key[0].upper()
    if first in "ABCDE": return "Core Nouns (A-E)"
    if first in "FGHIJ": return "Core Nouns (F-J)"
    if first in "KLMNO": return "Core Nouns (K-O)"
    if first in "PQRST": return "Core Nouns (P-T)"
    return "Core Nouns (U-Z)"
...
<body>
  ...
  <header class="shell site-header">
    ...
    <button class="hamburger" id="hamburgerBtn" aria-label="Open menu" aria-expanded="false" type="button">
      <span></span><span></span><span></span>
    </button>
    <nav class="nav-links" id="mainNav" aria-label="Primary navigation">
    ...
    <section id="home" class="section tab-panel">
      ...
      <div class="hp-hero">
        ...
        <div class="hp-hero-inner shell">
          ...
          <div class="wotd-card glass-panel" id="wotdCard">
            <span class="wotd-label">Word of the day</span>
            <div class="wotd-body">
              <span class="wotd-nari" id="wotdNari"></span>
              <span class="wotd-en" id="wotdEn"></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="hp-section">
        <div class="shell">
          <div class="hp-block glass-panel">
            ...
            <div class="hp-translate-hint">Unknown words are skipped.</div>
          </div>
        </div>
      </div>
    ...
    <section id="translator" class="section shell tab-panel">
      ...
      <div class="translator-grid">
        ...
        <div class="translator-output glass-panel" aria-live="polite">
          ...
          <button id="copyTranslationBtn" class="copy-btn" type="button" title="Copy to clipboard">&#128203; Copy</button>
        </div>
      </div>
    </section>
        ...
        <div class="practice-layout">
          <article class="practice-card glass-panel">
            <div class="build-header">
              ...
              <div class="practice-streak" id="drillStreak">&#128293; 0 streak</div>
            </div>
      ...
      <div id="clozePanel" class="practice-mode-panel">
        <div class="cloze-card glass-panel">
          <div class="build-header">
            ...
            <div class="practice-streak" id="clozeStreak">&#128293; 0 streak</div>
          </div>
      ...
      <div id="choicePanel" class="practice-mode-panel">
        <div class="choice-card glass-panel">
          <div class="build-header">
            ...
            <div class="practice-streak" id="choiceStreak">&#128293; 0 streak</div>
          </div>
        ...
        <div class="match-card glass-panel">
          <div class="build-header">
            <div style="display:flex; align-items:center; gap:0.5rem;">
              <p class="eyebrow" style="margin:0;">Word Match</p>
              <span id="matchScore" class="srs-badge" style="font-size:0.6rem; padding:0.15rem 0.4rem;">0 matched</span>
            </div>
            <div class="practice-streak" id="matchStreak">&#128293; 0 streak</div>
          </div>
      ...
      <div id="buildPanel" class="practice-mode-panel">
        <div class="build-card glass-panel">
          <div class="build-header">
            ...
            <div class="practice-streak" id="buildStreak">&#128293; 0 streak</div>
          </div>
...
@keyframes pulse-glow {
  0%, 100% { box-shadow: 0 0 10px rgba(121,225,211,0.2); }
  50% { box-shadow: 0 0 22px rgba(121,225,211,0.6); }
}
...
.translator-output { position: relative; padding-bottom: 3.5rem; }
...
.wotd-card {
  ...
  width: fit-content;
  max-width: 420px;
  background: linear-gradient(135deg, rgba(8,16,23,0.6), rgba(8,16,23,0.9));
  box-shadow: 0 8px 30px rgba(121,225,211,0.06);
}
.wotd-label {
  font-size: 0.7rem;
  text-transform: uppercase;
...
.wotd-body { display: flex; align-items: baseline; gap: 0.6rem; }
.wotd-nari { font-size: 1.4rem; font-weight: 700; color: var(--text); }
.wotd-en { font-size: 0.95rem; color: var(--muted); }
...
/* Builder header */
.build-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.5rem; }
.practice-streak { font-size: 0.85rem; font-weight: 700; color: var(--accent); }
...
function updateStreakDOM(id, count, key) {
  const el = document.querySelector("#" + id);
  if (el) el.innerHTML = `&#128293; ${count} streak`;
...
const buildStreak = document.querySelector("#buildStreak");
...
let drillState = "checking";
let drillStreakCount = parseInt(localStorage.getItem("nari_drill_streak") || "0");
window.addEventListener("DOMContentLoaded", () => updateStreakDOM("drillStreak", drillStreakCount, "nari_drill_streak"));
...
function checkCurrentDrill() {
  ...
  if (given === expected) {
    drillStreakCount++;
    updateStreakDOM("drillStreak", drillStreakCount, "nari_drill_streak");
    drillFeedback.textContent = `Correct: ${currentDrill.answer}`;
    drillFeedback.className = "is-correct";
    drillState = "next";
    checkDrill.textContent = "Next";
    nextDrill.style.display = "none";
    drillAnswer.disabled = true;
  } else {
    drillStreakCount = 0;
    updateStreakDOM("drillStreak", 0, "nari_drill_streak");
...
let matchMatched = 0;
let matchStreakCount = parseInt(localStorage.getItem("nari_match_streak") || "0");
window.addEventListener("DOMContentLoaded", () => updateStreakDOM("matchStreak", matchStreakCount, "nari_match_streak"));
...
matchGrid.addEventListener("click", (e) => {
  ...
  if (sameKey && diffSide) {
    ...
    matchMatched += 1;
    matchStreakCount++;
    updateStreakDOM("matchStreak", matchStreakCount, "nari_match_streak");
    ...
    matchStreakCount = 0;
    updateStreakDOM("matchStreak", 0, "nari_match_streak");
...
let buildStreakCount = parseInt(localStorage.getItem("nari_build_streak") || "0");
window.addEventListener("DOMContentLoaded", () => updateStreakDOM("buildStreak", buildStreakCount, "nari_build_streak"));
...
checkBuildBtn.addEventListener("click", () => {
  ...
  if (answer === currentBuildSentence.nari) {
    ...
    buildStreak.textContent = "\u{1F525} " + buildStreakCount + " streak";
    checkBuildBtn.style.display = "none";
    nextBuildBtn.style.display = "block";
  } else {
    buildStreakCount = 0;
    updateStreakDOM("buildStreak", 0, "nari_build_streak");
...
(function() {
  ...
  const entry = allWords[day % allWords.length];
  const nariEl = document.getElementById("wotdNari");
  const enEl = document.getElementById("wotdEn");
  if (nariEl && enEl && entry) {
    nariEl.textContent = entry.nari;
...
if (copyTranslationBtn) {
  copyTranslationBtn.addEventListener("click", () => {
    const text = document.getElementById("translatorOutput").textContent;
    navigator.clipboard.writeText(text).then(() => {
      copyTranslationBtn.textContent = "Copied!";
      window.setTimeout(() => { copyTranslationBtn.textContent = "\u{1F4CB} Copy"; }, 1500);
    }).catch(() => {
      copyTranslationBtn.textContent = "Error";
    });
  });
}

let clozeState = "checking";
let clozeStreakCount = parseInt(localStorage.getItem("nari_cloze_streak") || "0");
window.addEventListener("DOMContentLoaded", () => updateStreakDOM("clozeStreak", clozeStreakCount, "nari_cloze_streak"));

function loadCloze() {
  clozeState = "checking";
  checkCloze.textContent = "Check";
  nextCloze.style.display = "inline-block";
...
let choiceStreakCount = parseInt(localStorage.getItem("nari_choice_streak") || "0");
window.addEventListener("DOMContentLoaded", () => updateStreakDOM("choiceStreak", choiceStreakCount, "nari_choice_streak"));
function loadChoice() {
...
choiceOptions.addEventListener("click", (event) => {
  ...
  updateStreakDOM("choiceStreak", choiceStreakCount, "nari_choice_streak");
...
function checkClozeAnswer() {
  ...
  if (given === expected) {
    clozeStreakCount++;
    updateStreakDOM("clozeStreak", clozeStreakCount, "nari_cloze_streak");
    clozeFeedback.textContent = "Correct!";
    clozeFeedback.className = "is-correct";
    nextCloze.style.display = "none";
  } else {
    clozeStreakCount = 0;
    updateStreakDOM("clozeStreak", 0, "nari_cloze_streak");
