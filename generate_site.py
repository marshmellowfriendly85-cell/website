...
<body>
  ...
  <header class="shell site-header">
    ...
    <nav class="nav-links" aria-label="Primary navigation">
      <a href="#translator" class="is-active" data-tab-link="translator">Translator</a>
  ...
  <main id="top">
    ...
    <section id="translator" class="section shell tab-panel is-active">
    ...
    <section id="vocabulary" class="section shell tab-panel">
      ...
      <form id="customWordForm" class="custom-word-form glass-panel">
        <div>
          <label>
            <span>Nari word</span>
            <input id="customNari" type="text" autocomplete="off" placeholder="e.g. zuma">
          </label>
          <label>
            <span>English meaning</span>
            <input id="customEnglish" type="text" autocomplete="off" placeholder="e.g. charger">
          </label>
          <label>
            <span>Category</span>
            <input id="customCategory" type="text" autocomplete="off" placeholder="Custom">
          </label>
        </div>
        <button class="button primary" type="submit">Add word</button>
        <p id="customWordStatus">Custom words save in this browser and update the site instantly.</p>
      </form>
...
const customStorageKey = "nariCustomWords";
let customWords = loadCustomWords();
let rows = mergeRows(data.entries, customWords);
...
vocabRows.addEventListener("click", (event) => {
  const button = event.target.closest("[data-remove-word]");
  if (!button) return;
  const english = button.dataset.removeWord;
  customWords = customWords.filter((row) => row.english.toLowerCase() !== english.toLowerCase());
  saveCustomWords();
...
customWordForm.addEventListener("submit", (event) => {
  ...
  customWords = customWords.filter((row) => row.english.toLowerCase() !== english);
  customWords.push({ english, nari, category, custom: true });
  saveCustomWords();
  refreshVocabularyData();
  customWordForm.reset();
  customWordStatus.textContent = `Added ${nari} = ${english}.`;
});
...
function saveCustomWords() {
  localStorage.setItem(customStorageKey, JSON.stringify(customWords));
}
